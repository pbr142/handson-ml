...
class SimpleEncoder(BaseEstimator, TransformerMixin):
    """Simple contrast coding for the encoding of categorical features.
    Parameters
    ----------
    verbose: int
    ...
    >>> enc = SimpleEncoder(cols=['CHAS', 'RAD']).fit(X, y)
    ...
    References
    ----------
    .. [1] Simple Coding Systems for Categorical Variables, from
    ...
    def fit(self, X, y=None, **kwargs):
        ...
        for switch in ordinal_mapping:
            ...
            col = switch.get('col')
            column_mapping = self.fit_simple_coding(col, values, self.handle_missing, self.handle_unknown)
    ...
    def transform(self, X, override_return_df=False):
        ...
        X = self.simple_coding(X, mapping=self.mapping)
    ...
    @staticmethod
    def fit_simple_coding(col, values, handle_missing, handle_unknown):
        ...
        simple_contrast_matrix = Simple().code_without_intercept(values_to_encode.tolist())
        df = pd.DataFrame(data=simple_contrast_matrix.matrix, index=values_to_encode,
                          columns=[str(col) + '_%d' % (i, ) for i in range(len(simple_contrast_matrix.column_suffixes))])
    ...
    @staticmethod
    def simple_coding(X_in, mapping):
        """
        """
